...
<form id="add_new_product_form" enctype="multipart/form-data">
   ...
   <div class="row ">
      <div class="col-sm-12">
         <input type="submit"  class="btn btn-primary pull-right" name="add_new_product" id="add_new_product" style="margin-right:1%" value="Save" >
      </div>
   </div>
</form>
<script>
...
const putInput = () => {
  let html= `<div class="form-group mt-2">
   ...
   // This function is used to add new product
    $('form#add_new_product_form').on('submit', function(event){
         ...
         formData.append("supplierId", JSON.parse(localStorage.getItem('userInfo')).user.id)
 ...
 const addProductWithoutImages = (formData) => {
   ...
   const getCategories=()=>{
      $.ajax({
         type: 'GET',
         url: "{{ url('/api/tags/categories') }}"+"?supplierId="+ JSON.parse(localStorage.getItem('userInfo')).user.id,
         processData: false,
         contentType: false,
         dataType:'JSON',
         headers: {"Authorization": 'Bearer ' + localStorage.getItem('token')},
    ...
    const getSubCategories=()=>{
      $.ajax({
            type: 'GET',
            url: "{{ url('/api/tags/subcategories') }}"+"?supplierId="+ JSON.parse(localStorage.getItem('userInfo')).user.id,
            processData: false,
            contentType: false,
            dataType:'JSON',
            headers: {"Authorization": 'Bearer ' + localStorage.getItem('token')},
   ...
   const getCarousals=()=>{
    ...
    const getBrand=()=>{
      $.ajax({
            type: 'GET',
            url: "{{ url('/api/tags/brand') }}"+"?supplierId="+ JSON.parse(localStorage.getItem('userInfo')).user.id,
            processData: false,
            contentType: false,
            dataType:'JSON',
            headers: {"Authorization": 'Bearer ' + localStorage.getItem('token')},
   ...
   const addAttribute=(_this)=>{
      ...
      if(!checkattribute.includes(res[1]) ){
        ...
        html += '<button id="removeRow" value="'+res[1]+'" type="button" class="btn btn-primary pull-right">Remove</button>';
        html += '</div>';
        html += '</div>';
