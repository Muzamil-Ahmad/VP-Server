...
class CategoryController extends Controller
{
    ...
    public function create(Request $request){
        try {
           // dd($request->all());
            ...
            $check=DB::select("SELECT name FROM `categories` WHERE name='$request->name' AND is_deleted=0 AND supplier_id=$request->supplierId");
                if ($check==true) {
                    return response()->json([
            ...
            $res=DB::insert("INSERT INTO `categories` (`name`,`supplier_id`,`category_slug`,`banner`, `icon`,`alt_tag`) VALUES ('$request->name','$request->supplierId','$slug','$picture1','$picture2','$myJSON')");
            if ($res==true) {
                return response()->json([
    ...
    public function get(Request $request){
        ...
        $data = array();
           $totalData=\DB::table('categories')->where(['is_deleted'=>0])->where(['supplier_id'=>$request->query('supplierId')])->count();
