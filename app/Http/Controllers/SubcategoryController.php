...
class SubcategoryController extends Controller
{
     ...
     try
        {
            ...
            $check = DB::select("SELECT name FROM `subcategories` WHERE name='$request->name' AND supplier_id=$request->supplierId ");
            if ($check == true)
            {
            ...
            $res = DB::insert("INSERT INTO `subcategories` (`name`,`supplier_id`,`category_id`, `banner`, `icon`,`alt_tag`) VALUES ('$request->name','$request->supplierId','$request->fetchCat','$picture1','$picture2','$myJSON')");
            if ($res == true)
            {
    ...
    public function get(Request $request)
    {
        ...
        $data = array();
           $totalData=\DB::table('subcategories')->where(['is_deleted'=>0])->where(['supplier_id'=>$request->query('supplierId')])->count();
           $details=\DB::table('subcategories')->where(['is_deleted'=>0])
           ->where(['supplier_id'=>$request->query('supplierId')])
