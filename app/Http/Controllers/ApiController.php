...
class ApiController extends Controller
{
    ...
    public function getPopularProductsForBuyer(Request $data){
        ...
        foreach($data as $row)
        {
            ...
            $associate['attributes']=json_decode($row->attributes);
            $result[]=$associate;
        }
        // dd($result);
        if(count($result)>0){
    ...
    public function getProductBySlug($slug){
      // dd("hello");
        ...
        if(isset($data['slug'])){
            ...
            $associate['availability']=2;
            $associate['categories']=[];
            $associate['attributes']=json_decode($data->attributes);
      ...
      // dd($request->all());
        ...
        try {
            $order_id=DB::transaction(function () use($json_shipping_adress,$grand_total,$order_code,$order_date,$shipping_cost,$orders) {
                    $user = auth()->user();
                    $order_id = DB::table('orders')->insertGetId(['user_id'=>$user->id,'supplier_id'=>$orders[0]->supplier_id,'shipping_address'=>$json_shipping_adress,'grand_total'=>$grand_total,'code'=>$order_code,'payment_status'=>'unpaid','payment_details'=>'pending','shipping_cost'=>$shipping_cost,'created_at'=>$order_date,'updated_at'=>$order_date]);
                    //saving order data in order_details
                    foreach($orders as $order){
                        $quantity=$order->quantity;
                        $result=DB::table('order_details')->insertGetId(['order_id'=>$order_id,'supplier_id'=>$order->supplier_id,'product_id'=>$order->productId,'price'=>$order->product_discounted_price,'actual_price'=>$order->product_original_price,'quantity'=>$quantity,'tax'=>$order->product_tax,'discount'=>$order->product_discount,'shipping_cost'=>$shipping_cost,'shipping_type'=>'home delivery','created_at'=>$order_date,'updated_at'=>$order_date]);
