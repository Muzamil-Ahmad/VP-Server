...
class TagsController extends Controller
{
    ...
    {
      // dd($request->all());
        $details=\DB::table('categories')->where(['is_deleted'=>0])->where(['supplier_id'=>$request->query('supplierId')])->get();
           if($details) {
            foreach ($details as $key => $entity) {
                $nestedData['id'] = $entity->id;
    ...
    public function subcategoriesForAddProduct(Request $request)
    {
        ...
        $details=\DB::table('subcategories')->where(['is_deleted'=>0])->where(['supplier_id'=>$request->query('supplierId')])->get();
    ...
    {
        $details=\DB::table('brand')->where(['is_deleted'=>0])->where(['supplier_id'=>$request->query('supplierId')])->get();
           if($details) {
            foreach ($details as $key => $entity) {
                $nestedData['id'] = $entity->id;
                $nestedData['name'] = $entity->name;
